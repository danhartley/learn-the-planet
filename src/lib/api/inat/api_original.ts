...
import { iconicTaxa } from './inat-iconic-taxa'
...
export const getInatSpecies = async config => {
  ...
  const speciesNames = [] // TODO species names

  const snapdragonSpeciesNames = speciesNames[0].value

  //  console.log('snapdragon species', snapdragonSpeciesNames);
  ...
  const getIconicTaxa = config => {
    try {
      const iconicTaxa =
        config.guide.iconicTaxa.map(taxon => taxon.id) || iconicTaxaKeys
      ...
      console.error('Error for getIconicTaxa: ', e)
    }
  }
  ...
  const getAllInatObservations = async (config, snapdragonSpeciesNames) => {
    ...
    while (keepGoing) {
      try {
        ...
        let matches = recordsFromThisRequest.filter(record =>
          snapdragonSpeciesNames.includes(record.taxon.name)
        )
        ...
        let recordsCountReached =
          snapdragonSpecies.length >= config.guide.noOfRecords
        console.log('snapdragonSpecies', snapdragonSpecies)
        ...
        console.error('getInatObservations', e)
        return snapdragonSpecies
      }
    }
  }
    ...
    try {
      return Promise.all(
        observations.map(observation => {
          return [] // TODO species by name
          // return firestore
          //   .getSpeciesByName(observation.taxon.name)
          //   .then(async item => {
          //     return await {
          //       ...item,
      ...
      console.error('loadSpeciesInParallel: ', e)
    }
  }
  ...
  const getInatObservations = async (config, page) => {
    try {
      ...
      let radius = config.guide.speciesRange
        ? parseInt(config.guide.speciesRange)
        : 10
      let inat = ''
      ...
      const params =
        config.guide.guideTpe === 'INAT'
          ? getUserOrProjectKeyValuePair(config)
          : ''
      const url =
        getBasePath(config) +
        `&page=${page}&iconic_taxa=${iconicTaxa}&place_id=${placeId}&lat=${lat}&lng=${lng}&radius=${radius}${inat}${params}`

      console.log('inat species request url', url)

      const recordsFromThisRequest = await fetch(url)

      console.log('inat recordsFromThisRequest', recordsFromThisRequest)

      const json = await recordsFromThisRequest.json()
      inatListeners.forEach(listener =>
        listener({
          page: json.page,
          numberOfRequests: Math.ceil(
            json.total_results / json.RECORDS_per_page
          ),
        })
      )
  ...
  try {
    ...
    observations = observations.filter(observation =>
      snapdragonSpeciesNames.includes(observation.taxon.name)
    )
    sortBy(
      observations.filter(item => item),
      'observationCount',
      'desc'
    )

    // observations = take(
    //   config.guide.noOfRecords,
    //   utils.sortBy(
    //     observations.filter(item => item),
    //     'observationCount',
    //     'desc'
    ...
    console.error('getAllInatObservations', e)
  }
