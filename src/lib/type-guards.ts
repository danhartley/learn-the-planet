...
export const isTaxon = (obj: unknown): obj is Taxon => {
  ...
  const taxon = obj as unknown as Taxon
  ...
  for (const field of stringFields) {
    if (
      taxon[field as keyof Taxon] !== undefined &&
      typeof taxon[field as keyof Taxon] !== 'string'
    ) {
      return false
    }
  }
...
export const isNextCloudImage = (obj: unknown): obj is NextCloudImage => {
  ...
  const image = obj as unknown as NextCloudImage
